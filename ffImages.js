...
async function drawFreefire2TemplateImage(sortedTeams) {
  ...
  function drawText(x, y, str, { align = "center", color = "#ffffff", size = 15 } = {}) {
    ctx.font = `bold ${Math.round(size * scaleY)}px AppFont`;
    ctx.fillStyle = color;
    ctx.textAlign = align;
    let text = str;
  ...
  Object.entries(FREEFIRE2_ROWS_LEFT).forEach(([rank, y]) => {
    drawRow(FREEFIRE2_COLS_LEFT, y, sortedTeams[parseInt(rank) - 1], 15);
  });
  Object.entries(FREEFIRE2_ROWS_RIGHT).forEach(([rank, y]) => {
    drawRow(FREEFIRE2_COLS_RIGHT, y, sortedTeams[parseInt(rank) - 1], 15);
  });
