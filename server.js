...
// cho từng nhóm/chat. Admin gửi 1 ẢNH kèm chú thích (caption) đúng cú
// pháp "setqr Tên Chủ Khoản / Ngân Hàng / Số TK". Sau đó ai gõ "mã" hoặc
// "qr" trong nhóm/chat đó, bot trả lời đúng 3 dòng thông tin rồi gửi kèm
// CHÍNH ảnh QR đã lưu (không tự sinh QR nữa).
// ============================================================
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    // Ảnh admin đính kèm khi gõ "setqr ..." (dùng để LƯU LẠI QR thật, khác
    // với text — nên phải trích TRƯỚC khi return sớm vì tin ảnh có thể
    // không có text/rawText nào ngoài caption).
    const photoUrl = extractPhotoUrl(message);

    if (!chatId) return;
    // Cho phép đi tiếp nếu có text HOẶC có ảnh kèm caption "setqr" — các
    // trường hợp còn lại (ảnh không caption, sticker...) vẫn bỏ qua như cũ.
    if (!text) return;
    ...
    // Tương tự: nếu gõ "setqr ..." mà bot không tìm thấy ảnh đính kèm nào,
    // in nguyên văn message để biết chính xác field ảnh Zalo dùng là gì.
    if (/^setqr\b/i.test(text.trim()) && !photoUrl) {
      console.log(`[DEBUG] full message (setqr, no photo found)=${JSON.stringify(message)}`);
    }
...
async function handleMessage({ chatId, dataKey, pendingKey, fromId, text, isGroup, message }) {
  ...
  // ══════════════════════════════════════════════════════════
  // LỆNH ADMIN: SETQR — cấu hình QR chuyển khoản cho nhóm/chat hiện tại
  // Cú pháp: gửi 1 ẢNH kèm caption "setqr Tên Chủ Khoản / Ngân Hàng / Số TK"
  // VD caption: setqr NGUYỄN HỮU THÀNH NHÂN / ZALOPAY / 0948301012
  // ══════════════════════════════════════════════════════════
  {
    const m = /^setqr\s+(.+?)\s*\/\s*(.+?)\s*\/\s*(.+)$/i.exec(text.trim());
    if (m) {
      if (!isAdmin(fromId)) {
      ...
      if (!photoUrl) {
        return sendText(
          chatId,
          "❌ Chưa thấy ảnh đính kèm.\n" +
          "Gửi 1 ẢNH QR kèm chú thích (caption) đúng cú pháp:\n" +
          `setqr Tên Chủ Khoản / Ngân Hàng / Số TK`
        );
      }
      const [, name, bank, account] = m;
      setQrConfig(chatId, name.trim(), bank.trim(), account.trim(), photoUrl);
      return sendText(
        chatId,
        `✅ Đã lưu QR chuyển khoản cho box này:\n` +
        `👤 ${name.trim().toUpperCase()}\n` +
        `🏦 ${bank.trim().toUpperCase()}\n` +
        `🏧 ${account.trim()}\n\n` +
        `Từ giờ ai gõ "mã" hoặc "qr" trong box này sẽ nhận được thông tin + ảnh này.`
      );
    }
  }
  ...
  if (cmd === "help") {
    const adminHint = isAdmin(fromId)
      ? "\n\n👑 ADMIN — cấp/thu ngày: @Tên Bot add 7 <ID Zalo>  /  tru 3 <ID Zalo> (bảo họ nhắn \"whoami\" để lấy ID, cách chắc chắn nhất). Hoặc dùng tên hiển thị nếu họ đã từng nhắn tin trong nhóm.\n" +
        ...
        "👑 ADMIN — QR chuyển khoản: gửi 1 ẢNH kèm caption \"setqr Tên Chủ Khoản / Ngân Hàng / Số TK\". Sau đó ai gõ \"mã\" hoặc \"qr\" trong box sẽ nhận đủ thông tin + ảnh đó."
      : (isCtv(fromId)
