...
// ============================================================
// MẪU 16 — "BẢNG XẾP HẠNG" nền xanh Conan (Thám Tử Lừng Danh Conan).
// Có ô #1 đặc biệt bên trái (banner "TOP 1" + tên đội cạnh banner,
// số liệu BOOYAH!/ELIM/PTS nằm trong khung nhỏ phía dưới) và bảng
// #2-12 một cột bên phải. Toạ độ đo trực tiếp trên ảnh mẫu gốc
// 1364x768 (đo bằng cách quét pixel nhãn #2..#12, KHÔNG suy theo
// spacing đều vì khoảng cách dòng thực tế ~42-44px chứ không phải
// 38-39px như nhìn ước lượng ban đầu).
// ⚠️ Cần copy file ảnh gốc (conan_bxh_template.png) vào assets/templates/.
// ============================================================
const CONAN_TEMPLATE_PATH = path.join(__dirname, "assets", "templates", "conan_bxh_template.png");
let _conanImgCache = null;
async function getConanImage() {
  if (!_conanImgCache) _conanImgCache = await loadTemplateImage(CONAN_TEMPLATE_PATH, "Conan BXH");
  return _conanImgCache;
}

const CONAN_BASE_W = 1364;
const CONAN_BASE_H = 768;

// Ô #1 đặc biệt: tên đội căn baseline với chữ "TOP 1"; số BOOYAH! nằm
// cạnh logo; ELIM/PTS nằm dưới 2 nhãn "ELIM"/"PTS" trong khung nhỏ.
const CONAN_RANK1 = { teamX: 300, teamY: 440, byX: 445, byY: 536, elimX: 332, ptsX: 487, numY: 622 };

// Bảng #2-12 một cột, căn phải màn hình.
const CONAN_COLS = { team: 643, elim: 897, by: 983, pts: 1057 };
const CONAN_ROWS_Y = {
  2: 203, 3: 247, 4: 290, 5: 334, 6: 375, 7: 418,
  8: 461, 9: 503, 10: 545, 11: 587, 12: 630,
};

async function drawConanTemplateImage(sortedTeams) {
  const img = await getConanImage();
  const width = img.width;
  const height = img.height;
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");
  ...
  const scaleX = width / CONAN_BASE_W;
  const scaleY = height / CONAN_BASE_H;
  ctx.textBaseline = "middle";

  function drawText(x, y, str, { align = "center", color = "#ffffff", size = 20 } = {}) {
    ctx.font = `bold ${Math.round(size * scaleY)}px AppFont`;
    ctx.fillStyle = color;
    ctx.textAlign = align;
    let text = str;
  ...
  const top1 = sortedTeams[0];
  if (top1) {
    drawText(CONAN_RANK1.teamX, CONAN_RANK1.teamY, top1.rep || "Unknown", { align: "left", color: "#ffffff", size: 24 });
    drawText(CONAN_RANK1.byX, CONAN_RANK1.byY, String(top1.by ?? 0).padStart(2, "0"), { align: "left", color: "#ffffff", size: 22 });
    drawText(CONAN_RANK1.elimX, CONAN_RANK1.numY, String(top1.kill ?? 0), { align: "center", color: "#ffffff", size: 30 });
    drawText(CONAN_RANK1.ptsX, CONAN_RANK1.numY, String(top1.score ?? 0), { align: "center", color: "#ffdc3c", size: 30 });
  }

  Object.entries(CONAN_ROWS_Y).forEach(([rank, y]) => {
    const t = sortedTeams[parseInt(rank) - 1];
    if (!t) return;
    drawText(CONAN_COLS.team, y, t.rep || "Unknown", { align: "left", color: "#ffffff", size: 20 });
    drawText(CONAN_COLS.elim, y, String(t.kill ?? 0), { align: "center", color: "#ffffff", size: 19 });
    drawText(CONAN_COLS.by,   y, String(t.by ?? 0).padStart(2, "0"), { align: "center", color: "#ffffff", size: 19 });
    drawText(CONAN_COLS.pts,  y, String(t.score ?? 0), { align: "center", color: "#ffdc3c", size: 19 });
  });
...
// ============================================================
// MẪU 17 — "BẢNG XẾP HẠNG" nền xanh Kaito Kid — CÙNG LAYOUT hệt MẪU 16
// (Conan), chỉ khác ảnh nhân vật. Đã đo xác nhận lại toạ độ trên ảnh
// mẫu Kaito Kid, lệch 0-1px so với bản Conan nên dùng chung hệ số.
// ⚠️ Cần copy file ảnh gốc (kaitokid_bxh_template.png) vào assets/templates/.
// ============================================================
const KAITOKID_TEMPLATE_PATH = path.join(__dirname, "assets", "templates", "kaitokid_bxh_template.png");
let _kaitoKidImgCache = null;
async function getKaitoKidImage() {
  if (!_kaitoKidImgCache) _kaitoKidImgCache = await loadTemplateImage(KAITOKID_TEMPLATE_PATH, "Kaito Kid BXH");
  return _kaitoKidImgCache;
}

const KAITOKID_BASE_W = 1364;
const KAITOKID_BASE_H = 768;
const KAITOKID_RANK1 = CONAN_RANK1;
const KAITOKID_COLS = CONAN_COLS;
const KAITOKID_ROWS_Y = CONAN_ROWS_Y;

async function drawKaitoKidTemplateImage(sortedTeams) {
  const img = await getKaitoKidImage();
  const width = img.width;
  const height = img.height;
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");
  ...
  const scaleX = width / KAITOKID_BASE_W;
  const scaleY = height / KAITOKID_BASE_H;
  ctx.textBaseline = "middle";
  ...
  const top1 = sortedTeams[0];
  if (top1) {
    drawText(KAITOKID_RANK1.teamX, KAITOKID_RANK1.teamY, top1.rep || "Unknown", { align: "left", color: "#ffffff", size: 24 });
    drawText(KAITOKID_RANK1.byX, KAITOKID_RANK1.byY, String(top1.by ?? 0).padStart(2, "0"), { align: "left", color: "#ffffff", size: 22 });
    drawText(KAITOKID_RANK1.elimX, KAITOKID_RANK1.numY, String(top1.kill ?? 0), { align: "center", color: "#ffffff", size: 30 });
    drawText(KAITOKID_RANK1.ptsX, KAITOKID_RANK1.numY, String(top1.score ?? 0), { align: "center", color: "#ffdc3c", size: 30 });
  }

  Object.entries(KAITOKID_ROWS_Y).forEach(([rank, y]) => {
    const t = sortedTeams[parseInt(rank) - 1];
    if (!t) return;
    drawText(KAITOKID_COLS.team, y, t.rep || "Unknown", { align: "left", color: "#ffffff", size: 20 });
    drawText(KAITOKID_COLS.elim, y, String(t.kill ?? 0), { align: "center", color: "#ffffff", size: 19 });
    drawText(KAITOKID_COLS.by,   y, String(t.by ?? 0).padStart(2, "0"), { align: "center", color: "#ffffff", size: 19 });
    drawText(KAITOKID_COLS.pts,  y, String(t.score ?? 0), { align: "center", color: "#ffdc3c", size: 19 });
  });
...
// ============================================================
// MẪU 18 — "BẢNG XẾP HẠNG TỔNG" nền đỏ-vàng Tết. Top 1-3 là 3 thẻ
// riêng bên trái (thẻ #1 nền vàng → chữ TỐI; #2-#3 nền đỏ → chữ
// TRẮNG, mỗi thẻ cao ~66-67px với nhãn ELIM/BOOYAH!/PTS nằm ở nửa
// trên và số liệu nằm ngay dưới nhãn đó); #4-12 là bảng một cột bên
// phải. Toạ độ đo trực tiếp trên ảnh mẫu gốc 1364x768.
// ⚠️ Cần copy file ảnh gốc (tet_bxh_template.png) vào assets/templates/.
// ============================================================
const TETBXH_TEMPLATE_PATH = path.join(__dirname, "assets", "templates", "tet_bxh_template.png");
let _tetBxhImgCache = null;
async function getTetBxhImage() {
  if (!_tetBxhImgCache) _tetBxhImgCache = await loadTemplateImage(TETBXH_TEMPLATE_PATH, "BXH Tết");
...
// 3 thẻ #1-#3 bên trái — mỗi thẻ có toạ độ Y riêng (không đều nhau vì
// thẻ #1 cao hơn 1 chút do có banner "TOP 1" phía trên).
const TETBXH_LEFT_COLS = { team: 225, elim: 460, by: 545, pts: 605 };
const TETBXH_TOP3 = {
  1: { nameY: 402, numY: 415, color: "#2a1400" }, // thẻ #1 nền vàng → chữ tối
  2: { nameY: 469, numY: 480, color: "#ffffff" },
  3: { nameY: 536, numY: 547, color: "#ffffff" },
};

// Bảng #4-12 một cột bên phải.
const TETBXH_RIGHT_COLS = { team: 750, elim: 980, by: 1092, pts: 1195 };
const TETBXH_ROWS_Y = {
  4: 212, 5: 260, 6: 306, 7: 354, 8: 401,
  9: 448, 10: 495, 11: 542, 12: 589,
};

async function drawTetBxhTemplateImage(sortedTeams) {
  ...
  function drawText(x, y, str, { align = "center", color = "#ffffff", size = 19 } = {}) {
    ctx.font = `bold ${Math.round(size * scaleY)}px AppFont`;
    ctx.fillStyle = color;
    ctx.textAlign = align;
    let text = str;
  ...
  // 3 thẻ #1-#3
  Object.entries(TETBXH_TOP3).forEach(([rank, cfg]) => {
    const t = sortedTeams[parseInt(rank) - 1];
    if (!t) return;
    drawText(TETBXH_LEFT_COLS.team, cfg.nameY, t.rep || "Unknown", { align: "left", color: cfg.color, size: 19 });
    drawText(TETBXH_LEFT_COLS.elim, cfg.numY, String(t.kill ?? 0), { align: "center", color: cfg.color, size: 22 });
    drawText(TETBXH_LEFT_COLS.by,   cfg.numY, String(t.by ?? 0), { align: "center", color: cfg.color, size: 22 });
    drawText(TETBXH_LEFT_COLS.pts,  cfg.numY, String(t.score ?? 0), { align: "center", color: cfg.color, size: 22 });
  });

  // Bảng #4-12
  Object.entries(TETBXH_ROWS_Y).forEach(([rank, y]) => {
    const t = sortedTeams[parseInt(rank) - 1];
    if (!t) return;
    drawText(TETBXH_RIGHT_COLS.team, y, t.rep || "Unknown", { align: "left", color: "#ffffff", size: 20 });
    drawText(TETBXH_RIGHT_COLS.elim, y, String(t.kill ?? 0), { align: "center", color: "#ffffff", size: 19 });
    drawText(TETBXH_RIGHT_COLS.by,   y, String(t.by ?? 0), { align: "center", color: "#ffffff", size: 19 });
    drawText(TETBXH_RIGHT_COLS.pts,  y, String(t.score ?? 0), { align: "center", color: "#ffdc3c", size: 19 });
  });
...
module.exports = {
  ...
  drawConanTemplateImage,
  drawKaitoKidTemplateImage,
  drawTetBxhTemplateImage,
};
