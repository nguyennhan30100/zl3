...
async function handleMessage({ chatId, dataKey, pendingKey, fromId, text, isGroup, message }) {
  ...
  if (pending?.step === "date") {
    ...
    return sendText(
      ...
      "Nhập số từ 1-43" + groupHint
    );
  }

  // BƯỚC 4: chọn mẫu ảnh → chạy toàn bộ luồng lấy điểm
  if (pending?.step === "template") {
    if (!/^([1-9]|[1-3][0-9]|4[0-3])$/.test(text)) return sendText(chatId, `❌ Chỉ nhập số từ 1 đến 43!${groupHint}`);
    ...
    async function drawSelectedTemplate(sortedTeams, subtitleText) {
      if (selectedTemplate === 43) return drawFfwsSeaTemplateImage(sortedTeams);
      if (selectedTemplate === 42) return drawBlueRedSoccerTemplateImage(sortedTeams);
      if (selectedTemplate === 41) return drawRedDemon2PanelTemplateImage(sortedTeams);
      if (selectedTemplate === 40) return drawPink2PanelTemplateImage(sortedTeams);
      if (selectedTemplate === 39) return drawBlue2PanelTemplateImage(sortedTeams);
      if (selectedTemplate === 38) return drawTodayStandingTemplateImage(sortedTeams);
      if (selectedTemplate === 37) return drawSpaceAnimeOverallStandingsTemplateImage(sortedTeams);
      if (selectedTemplate === 36) return drawPurpleMhaTemplateImage(sortedTeams);
      if (selectedTemplate === 35) return drawOnepiece2TemplateImage(sortedTeams);
      if (selectedTemplate === 34) return drawGreenBxhTemplateImage(sortedTeams);
      if (selectedTemplate === 33) return drawNeonOverallStandingsTemplateImage(sortedTeams);
      if (selectedTemplate === 32) return drawFfScrimYellowTemplateImage(sortedTeams);
      if (selectedTemplate === 31) return drawDemonslayerTemplateImage(sortedTeams);
      if (selectedTemplate === 30) return drawAnimeBlueBxhTongTemplateImage(sortedTeams);
      if (selectedTemplate === 29) return drawSoldierTemplateImage(sortedTeams);
      if (selectedTemplate === 28) return drawDragonballTemplateImage(sortedTeams);
      if (selectedTemplate === 27) return drawJjkTemplateImage(sortedTeams);
      if (selectedTemplate === 26) return drawCyberCatTemplateImage(sortedTeams);
      if (selectedTemplate === 25) return drawOverallStandingRedTemplateImage(sortedTeams);
      if (selectedTemplate === 24) return drawPinkAnimeTemplateImage(sortedTeams);
      if (selectedTemplate === 23) return drawPurpleCatTemplateImage(sortedTeams);
      if (selectedTemplate === 22) return drawOverallStandingsAnimeTemplateImage(sortedTeams);
  ...
  if (cmd === "help") {
    return sendText(
      ...
      "⚙️ Luồng: .td [ID] → chọn khung giờ (1-8) → nhập ngày → chọn mẫu ảnh (1-43) → bot tự lấy điểm.\n\n" +
