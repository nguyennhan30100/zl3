...
async function handleMessage({ chatId, dataKey, pendingKey, fromId, text, isGroup, message }) {
  ...
  if (pending?.step === "date") {
    ...
    return sendText(
      ...
      "4 — Mẫu Hello Kitty\n" +
      "5 — Mẫu Xích Sắt\n" +
      "6 — Mẫu Free Fire Bảng Xếp Hạng\n" +
      "7 — Mẫu Free Fire 2 Cột\n\n" +
      "Trả lời 1, 2, 3, 4, 5, 6 hoặc 7" + groupHint
    );
  }

  // BƯỚC 4: chọn mẫu ảnh → chạy toàn bộ luồng lấy điểm
  if (pending?.step === "template") {
    if (!/^[1234567]$/.test(text)) return sendText(chatId, `❌ Chỉ nhập 1, 2, 3, 4, 5, 6 hoặc 7!${groupHint}`);
  ...
  if (cmd === "help") {
    return sendText(
      ...
      "⚙️ Luồng: .td [ID] → chọn khung giờ (1-8) → nhập ngày → chọn mẫu ảnh (1-7) → bot tự lấy điểm.\n\n" +
