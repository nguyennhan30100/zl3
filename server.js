...
async function handleMessage({ chatId, dataKey, pendingKey, fromId, text, isGroup, message }) {
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    if (m) {
      ...
      return sendText(
        ...
        `🏧 ${account.trim()}\n\n` +
        `📸 Gửi tiếp 1 ẢNH QR (không cần caption) để hoàn tất cấu hình.${groupHint}`
      );
    }
  }
  ...
  if (pending?.step === "setqr_photo") {
    ...
    if (!photoUrl) {
      return sendText(
        chatId,
        `❌ Chưa thấy ảnh. Gửi 1 ẢNH QR để hoàn tất cấu hình.${groupHint}`
      );
    }
    const { name, bank, account } = pending;
    delete pendingStep[pendingKey];
    setQrConfig(chatId, name, bank, account, photoUrl);
    await sendText(
      chatId,
  ...
  if (cmd === "help") {
    const adminHint = isAdmin(fromId)
      ? "\n\n👑 ADMIN — cấp/thu ngày: @Tên Bot add 7 <ID Zalo>  /  tru 3 <ID Zalo> (bảo họ nhắn \"whoami\" để lấy ID, cách chắc chắn nhất). Hoặc dùng tên hiển thị nếu họ đã từng nhắn tin trong nhóm.\n" +
        ...
        "👑 ADMIN — QR chuyển khoản: gõ \"setqr Tên Chủ Khoản / Ngân Hàng / Số TK\" → bot xác nhận → gửi tiếp 1 ẢNH QR (không cần caption) → bot tự lưu. Sau đó ai gõ \"mã\" hoặc \"qr\" trong box sẽ nhận đủ thông tin + ảnh đó."
      : (isCtv(fromId)
