...
async function handleMessage({ chatId, dataKey, pendingKey, fromId, text, isGroup, message }) {
  ...
  if (pending?.step === "date") {
    ...
    return sendText(
      ...
      "Nhập số từ 1-41" + groupHint
    );
  }

  // BƯỚC 4: chọn mẫu ảnh → chạy toàn bộ luồng lấy điểm
  if (pending?.step === "template") {
    if (!/^([1-9]|[1-3][0-9]|4[0-1])$/.test(text)) return sendText(chatId, `❌ Chỉ nhập số từ 1 đến 41!${groupHint}`);
  ...
  if (cmd === "help") {
    return sendText(
      ...
      "⚙️ Luồng: .td [ID] → chọn khung giờ (1-8) → nhập ngày → chọn mẫu ảnh (1-41) → bot tự lấy điểm.\n\n" +
