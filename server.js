...
async function handleMessage({ chatId, dataKey, pendingKey, fromId, text, isGroup, message }) {
  ...
  if (pending?.step === "date") {
    ...
    return sendText(
      ...
      "Nhập số từ 1-18" + groupHint
    );
  }

  // BƯỚC 4: chọn mẫu ảnh → chạy toàn bộ luồng lấy điểm
  if (pending?.step === "template") {
    if (!/^([1-9]|1[0-8])$/.test(text)) return sendText(chatId, `❌ Chỉ nhập số từ 1 đến 18!${groupHint}`);
    ...
    async function drawSelectedTemplate(sortedTeams, subtitleText) {
      if (selectedTemplate === 18) return drawTetBxhTemplateImage(sortedTeams);
      if (selectedTemplate === 17) return drawKaitoKidTemplateImage(sortedTeams);
      if (selectedTemplate === 16) return drawConanTemplateImage(sortedTeams);
  ...
  if (cmd === "help") {
    return sendText(
      ...
      "⚙️ Luồng: .td [ID] → chọn khung giờ (1-8) → nhập ngày → chọn mẫu ảnh (1-18) → bot tự lấy điểm.\n\n" +
    ...
    if (isAdmin(fromId)) {
      ...
      sections.push({
        ...
        body:
          ...
          "(Bạn không có quyền thu ngày — lệnh \"tru\" chỉ admin dùng được)\n\n" +
          "id — Trả lời (Reply/Quote) tin nhắn của ai đó rồi gõ \"id\" để lấy ID Zalo của họ",
      });
    }
