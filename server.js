...
async function handleMessage({ chatId, dataKey, pendingKey, fromId, text, isGroup, message }) {
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    if (m) {
      ...
      // Lớp 1 & 2: thử Reply / entities (hiện Zalo chưa gửi, xem ghi chú ở
      ...
      // Lớp 3: tra "sổ danh bạ" tự học theo tên còn lại sau "add N " /
      ...
      if (!targetId) {
        return sendText(
          ...
          `Gõ đúng tên hiển thị Zalo của họ, VD: "${action} ${days} Tên Người Đó" ` +
          "(không cần dấu @, không cần Reply — chỉ cần người đó ĐÃ TỪNG nhắn ít nhất 1 tin trong nhóm này).\n" +
          "Nếu họ chưa từng nhắn gì trong nhóm, nhờ họ nhắn 1 câu bất kỳ rồi thử lại."
        );
      }
  ...
  if (cmd === "help") {
    return sendText(
      ...
      "👑 ADMIN — cấp/thu ngày: @Tên Bot add 7 Tên Người Đó (gõ đúng tên hiển thị Zalo, không cần @ hay Reply, chỉ cần người đó đã từng nhắn ít nhất 1 tin trong nhóm)."
    );
  }
